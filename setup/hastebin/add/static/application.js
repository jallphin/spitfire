...
haste.prototype.loadDocument = function(key) {
  ...
  _this.doc.load(parts[0], function(ret) {
    if (ret) {
      var ext = parts[1];
      if (ext)
        _this.$code.attr('class', _this.lookupTypeByExtension(ext));

      var c = _this.$code[0];
      c.appendChild(document.createTextNode(ret.value));
      hljs.highlightBlock(c)
      console.log("Detected language: " + c.result.language + "(" + c.result.re + ")")
      if (c.second_best)
        console.log("Second guess: " + c.second_best.language + "(" + c.second_best.re + ")")
      if (!ext)
	window.location.pathname += "." + c.result.language;
